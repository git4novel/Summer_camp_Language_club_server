...
const verifyJWT = (req, res, next) => {
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
    if (err) {
      return res
        .status(401)
        .send({ error: true, message: "unauthorized credential" });
    }
    req.decoded = decoded;
    next();
  });
...
async function run() {
  try {
    ...
    const studentAddedClassCollection = client
      .db("summerCamp")
      .collection("studentAddedClass");

    // will use on payment completion
    ...
    const verifyAdmin = async (req, res, next) => {
      ...
      if (user?.role !== "admin") {
        return res
          .status(403)
          .send({ error: true, message: "forbidden response" });
      }
      next();
    };
    ...
    // -------
    // all added class for admin to see
    ...
    // -----------------------------------
    // here admin work for instructor made class approve deny adn feedback
    ...
    app.patch("/classes/feedback/:id", async (req, res) => {
      const feedbackMessage = req.body.message; // Retrieve the feedback message from the request body
